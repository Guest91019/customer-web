...
// เก็บ role
export function saveRole(uid, role) {
  return set(ref(db, "users/" + uid), { role: role });
}

// โหลด role
export function getRole(uid, callback) {
  onValue(ref(db, "users/" + uid), (snapshot) => {
    if (snapshot.exists()) callback(snapshot.val().role);
  });
}

// บันทึกออเดอร์
export function saveOrder(name, order) {
  const newRef = push(ref(db, "orders"));
  return set(newRef, {
    customer: name,
    order: order,
...
export function loadOrders(callback) {
  onValue(ref(db, "orders"), (snapshot) => {
    const list = [];
    snapshot.forEach(child => list.push({ key: child.key, ...child.val() }));
    callback(list);
  });
}

// อัปเดต / ลบ (manager)
export function updateOrder(key, newData) {
  return update(ref(db, "orders/" + key), newData);
}

export function deleteOrder(key) {
  return remove(ref(db, "orders/" + key));
}
